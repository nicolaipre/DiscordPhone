...
# Discord imports
import discord # ( python3 -m pip install https://github.com/imayhaveborkedit/discord.py/archive/voice-recv-mk2.zip )
import asyncio
import ctypes
import ctypes.util
...
# Receive audio from microphone source/input - Credits to gerth2: https://github.com/RobotCasserole1736/CasseroleDiscordBotPublic/blob/master/audioHandling.py
...
# Send audio to speaker sink/output
...
######################
"""
The following two methods are currently not in use, but will be used if Linphone audio can be imported to this application somehow...
"""

# Send audio to buffer sink/output
class BufferAudioSink(discord.reader.AudioSink):
    def __init__(self, duration_ms=20):
    ...
    def freshen(self, index):
        self.audioData = self.audioData[index:]




# Send audio to buffer source/input
# TODO: Fix this one
class BufferAudioSource(discord.PCMAudio):
    def __init__(self, buffer, duration_ms=20):
        self.audioData = buffer
        self.SAMPLE_WIDTH = 2
        self.SAMPLE_RATE = 48000

    def read(self):
        retVal = self.audioData.read(self.NUM_SAMPLES)
        rawData = bytes(retVal[0])
        return rawData
...
class Anna(discord.Client):
    ...
    async def call(self, phone_number, spoofed_number):
        ...
        # 2. relay mic and speakers
        # 3. call
        # 4. leave when call finishes
